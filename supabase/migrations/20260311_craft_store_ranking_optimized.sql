-- Optimized store ranking using CTEs to scan the materialized view only twice instead of 4 times.
...
BEGIN
    ...
    SELECT json_build_object(
        'top_stores', (
            ...
            FROM (SELECT * FROM store_agg ORDER BY total_sold DESC LIMIT 5) t
        ),
        'bottom_stores', (
            SELECT COALESCE(json_agg(row_to_json(b)), '[]'::JSON)
            FROM (SELECT * FROM store_agg ORDER BY total_sold ASC LIMIT 5) b
        ),
        'all_stores', (
            SELECT COALESCE(json_agg(row_to_json(a)), '[]'::JSON)
            FROM (SELECT * FROM store_agg ORDER BY total_sold DESC) a
        ),
        'sku_abc', (
            SELECT COALESCE(json_agg(row_to_json(s)), '[]'::JSON)
            FROM (SELECT * FROM sku_agg ORDER BY total_revenue DESC) s
        )
    ) INTO result;
