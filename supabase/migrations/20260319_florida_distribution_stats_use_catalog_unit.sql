CREATE OR REPLACE VIEW florida1.v_florida_distribution_stats_catalog_14d AS
WITH params AS (
...
catalog_products AS (
    ...
    FROM florida1.production_180d_products p180
...
stock_now_by_spot_product AS (
    ...
    LEFT JOIN florida1.effective_stocks es
...
today_production AS (
    SELECT
        v_florida_production_only.product_id::bigint AS product_id,
        MAX(v_florida_production_only.product_name) AS product_name,
        SUM(v_florida_production_only.baked_at_factory)::integer AS baked_at_factory
    FROM florida1.v_florida_production_only
    GROUP BY v_florida_production_only.product_id::bigint
),
base AS (
    SELECT
        cp.product_id,
...
SELECT
    ...
    CEIL(GREATEST(0::numeric, n.avg_sales_day) * 3)::integer AS min_stock,
    ROUND(GREATEST(0::numeric, n.stock_now), 0)::integer AS stock_now,
    GREATEST(0, COALESCE(n.baked_at_factory, 0)) AS baked_at_factory,
    ROUND(
        GREATEST(
            0::numeric,
            CEIL(GREATEST(0::numeric, n.avg_sales_day) * 3) - GREATEST(0::numeric, n.stock_now)
        ),
        0
    )::integer AS need_net,
    n.spot_id,
