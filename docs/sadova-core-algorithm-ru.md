...
- **Entities**: Хранят текущее состояние. В отличие от Гравитона, здесь нет сущности `delivery_debt` — долги не фиксируются и не переносятся.
- **Use Cases**: Основная бизнес-логика. Сбор живых остатков с Poster, вызов `fn_orchestrate_distribution_live` и сам математический расчет `fn_run_distribution_v4`.
- **Interface Adapters**: Next.js API, которые собирают данные с Poster Edge Functions и передают в Supabase RPC.
...
## 3. Отличия от Гравитона (Отсутствие Долгов)

В Садове **полностью отсутствует механизм `delivery_debt`**. Система работает в режиме "здесь и сейчас".

1. Расчет потребности выполняется только на базе текущих продаж и остатков: `min_stock - stock`.
2. Если логист не привез продукцию или машина сломалась, система не запомнит этот факт в виде долга и не начислит компенсацию при следующем расчете.
3. Это делает алгоритм проще, снижает риск "человеческого фактора" (Human Error) при ручном проставлении галочки подтверждения доставки, но требует более жесткого контроля дисциплины водителей.
