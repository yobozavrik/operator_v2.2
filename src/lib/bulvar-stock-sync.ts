...
export interface BulvarStockSyncResult {
    syncedRows: number;
...
export interface BulvarEdgeStockRow {
    storage_id: number;
    spot_id: number;
    spot_name: string;
...
export interface BulvarEdgeStocksResult {
    rows: BulvarEdgeStockRow[];
    successfulStorageIds: number[];
    skippedStorages: number[];
    warnings: string[];
}

interface BulvarSpotMeta {
    spot_id: number;
    spot_name: string;
}
...
async function buildBulvarStorageToSpotMap(
    supabase: SupabaseClient
): Promise<Map<number, BulvarSpotMeta>> {
    ...
    const spotByNormalizedName = new Map<string, BulvarSpotMeta>();
    ...
    const storageToSpot = new Map<number, BulvarSpotMeta>();
...
export async function fetchBulvarEdgeStocks(
    supabase: SupabaseClient
): Promise<BulvarEdgeStocksResult> {
    const warnings: string[] = [];
    const skippedStorages = new Set<number>();
    const successfulStorages = new Set<number>();

    const storageToSpot = await buildBulvarStorageToSpotMap(supabase);
    const storageIds = Array.from(storageToSpot.keys());

    if (storageIds.length === 0) {
        warnings.push('No mapped Bulvar storages found for edge sync.');
        return {
            rows: [],
    ...
    if (!edgeResponse.ok) {
        const details =
            typeof edgePayload?.error === 'string'
                ? edgePayload.error
                : edgePayload?.error?.message || `HTTP ${edgeResponse.status}`;
    ...
    const normalizedRows: BulvarEdgeStockRow[] = [];
...
export async function syncBulvarStocksFromEdge(
    supabase: SupabaseClient
): Promise<BulvarStockSyncResult> {
    ...
    if (upsertRows.length > 0) {
        for (const chunk of chunkArray(upsertRows, 1000)) {
            ...
            if (error) {
                throw new Error(`Failed to upsert bulvar1.effective_stocks: ${error.message}`);
            }
        }
    }
    ...
    if (cleanupStorageIds.length > 0) {
        ...
        if (cleanupError) {
            throw new Error(`Failed to cleanup stale bulvar1.effective_stocks rows: ${cleanupError.message}`);
        }
    }
