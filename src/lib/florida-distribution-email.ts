...
export interface FloridaDistributionEmailRow {
    product_name: string;
    spot_name: string;
...
export interface SendFloridaDistributionEmailInput {
    businessDate: string;
    rows: FloridaDistributionEmailRow[];
    productionRowsCount: number;
}

export interface SendFloridaDistributionEmailResult {
    sent: boolean;
    skipped: boolean;
...
<h2 style="margin:0 0 12px 0;">Florida distribution ${businessDate}</h2>
...
export async function sendFloridaDistributionEmail(
    input: SendFloridaDistributionEmailInput
): Promise<SendFloridaDistributionEmailResult> {
    const env = getDistributionEmailEnv('florida');
    ...
    const excelBuffer = await buildDistributionExcelBuffer('Florida', input.businessDate, input.rows);
    ...
    try {
        const response = await fetch('https://api.resend.com/emails', {
            ...
            body: JSON.stringify({
                ...
                attachments: [
                    {
                        filename: `florida-distribution-${input.businessDate}.xlsx`,
