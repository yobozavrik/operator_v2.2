import { normalizeGravitonName } from '@/lib/graviton-catalog';
...
export interface GravitonEdgeProductionRow {
...
export function extractGravitonEdgeProduction(
    edgePayload: unknown,
    productionStorageId: number | null = 2
): GravitonEdgeProductionRow[] {
        ...
        .map((row) => {
            ...
            return {
                ...
                product_name_normalized:
                    String(row.product_name_normalized || '').trim() || normalizeGravitonName(productName),
                quantity,
            };
        })
        .filter((row): row is GravitonEdgeProductionRow => row !== null);
}
