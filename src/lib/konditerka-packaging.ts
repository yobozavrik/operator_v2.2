...
export interface KonditerkaPackagingConfig {
    product_id: number;
...
export async function fetchKonditerkaPackagingConfig(
    supabase: SupabaseClient,
    productIds: number[]
): Promise<Map<number, KonditerkaPackagingConfig>> {
    ...
    return new Map(
        (data || []).map((row: Record<string, unknown>) => [
            ...
            } satisfies KonditerkaPackagingConfig,
        ])
    );
}

export function estimatePackagingPacks(kg: number, config: KonditerkaPackagingConfig): number {
...
export function applyKonditerkaPackagingConfigToRows<T extends DistributionLikeRow>(
    rows: T[],
    configMap: Map<number, KonditerkaPackagingConfig>,
