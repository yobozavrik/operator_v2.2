...
export async function getCategories() {
    const stop = startTimer('Poster API fetch categories');
    const categoriesData = await posterRequest('menu.getCategories').finally(stop);
    return categoriesData.response || [];
}

export async function getProducts() {
    const stop = startTimer('Poster API fetch products');
    const productsData = await posterRequest('menu.getProducts').finally(stop);
...
export async function getAllLeftovers(
    ...
    const stopStorages = startTimer('Poster API fetch storages');
    const storagesData = await posterRequest('storage.getStorages').finally(stopStorages);
    ...
    const stopLeftovers = startTimer('Poster API fetch leftovers parallel');
    ...
    const results = await Promise.all(promises).finally(stopLeftovers);
...
export async function getTodayManufactures(
    ...
    const stopManufactures = startTimer('Poster API fetch manufactures');
