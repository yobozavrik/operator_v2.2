...
interface BulvarProductionDetailModalProps {
    isOpen: boolean;
    onClose: () => void;
}
...
export const BulvarProductionDetailModal = ({ isOpen, onClose }: BulvarProductionDetailModalProps) => {
    const { data, error, isLoading } = useSWR<ProductionItem[]>(
        isOpen ? '/api/bulvar/production-detail' : null,
                ...
                {isLoading ? (
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead className="sticky top-0 bg-panel-bg z-10 text-[10px] uppercase font-bold tracking-widest text-text-muted border-b border-panel-border">
                            <tr>
                                <th className="p-4 pl-6">Бульвар-Автовокзал</th>
