                    ...
                    return (
                        ...
                        >
                            ...
                            <div className="text-xs font-bold uppercase tracking-wide truncate saas-text-primary text-center" title={store.storeName}>
                                {store.storeName.replace('РњР°РіР°Р·РёРЅ ', '').replace('"', '').replace('"', '')}
                            </div>

                            {/* Metrics Row - Compact */}
                            <div className="grid grid-cols-3 gap-1 text-[10px] font-mono leading-none mt-0.5">
                                <div className="flex flex-col gap-0.5">
                                    <span className="text-[9px] saas-text-secondary uppercase font-bold">Р¤Р°РєС‚</span>
                                    <span className={cn("font-bold text-lg", isLowStock ? "text-[#E74856]" : "text-emerald-500")}>
                                        {formatStock(store.computed.stock, product.unit)}
                                    </span>
                                </div>
                                <div className="flex flex-col gap-0.5 text-center">
                                    <span className="text-[9px] saas-text-secondary uppercase font-bold">РњС–РЅ</span>
                                    <span className="font-bold text-lg text-[#2b80ff]">{formatMetric(store.computed.minStock, product.unit)}</span>
                                </div>
                                <div className="flex flex-col gap-0.5 text-right">
                                    <span className="text-[9px] saas-text-secondary uppercase font-bold">РЎРµСЂ</span>
                            ...
                            <div className="pt-1.5 border-t border-slate-100 mt-0.5 flex items-center justify-between">
                                <span className="text-[9px] text-amber-500 font-bold uppercase">РџР»Р°РЅ</span>
                                <input
                                    type="number"
                                    value={planVal || ''}
                                    onChange={(e) => {
            ...
            <div className="p-3 bg-slate-50 rounded-lg border border-slate-200 flex items-center gap-4">
                <div className="flex-1 max-w-xs">
                    <label className="text-[10px] saas-text-secondary uppercase font-bold tracking-widest block mb-1">
                        РЎРєС–Р»СЊРєРё РІРёРїРµС‡РµРЅРѕ (РѕРґ.)
                    </label>
                    <input
                        type="number"
                        value={totalBaked || ''}
                ...
                >
                    ...
                    Р РѕР·РїРѕРґС–Р»РёС‚Рё
                </button>

                <div className="ml-auto mt-4 text-right">
                    <div className="text-[10px] saas-text-secondary uppercase tracking-widest font-bold">РќРµСЂРѕР·РїРѕРґС–Р»РµРЅРѕ</div>
...
export const BulvarPowerMatrix = ({ data, onRefresh, initialViewMode = 'products' }: Props) => {
    ...
    const isKgUnit = (unit: unknown) => {
        const normalized = String(unit || '').trim().toLowerCase();
        return normalized === 'кг' || normalized === 'kg';
    };
    const formatStock = (value: number, unit: unknown) => {
        const safe = Number.isFinite(value) ? value : 0;
        return isKgUnit(unit) ? safe.toFixed(2) : safe.toFixed(0);
    };
    const formatMetric = (value: number, unit: unknown) => {
        const safe = Number.isFinite(value) ? value : 0;
        return isKgUnit(unit) ? safe.toFixed(1) : safe.toFixed(0);
    };
    ...
    // рџ”Ґ WEBHOOK: Update stock from n8n
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    ...
    const products = useMemo(() => {
        return data.map(product => {
            ...
            const storesWithStats = product.stores.map(store => {
                ...
                const rowUnit = store.unit || product.unit;
                ...
                const rawTarget = (avg * planningDays) + minStock;
                const target = isKgUnit(rowUnit) ? rawTarget : Math.ceil(rawTarget);
                ...
                return {
                    ...
                    packaging_enabled: Boolean((store as any).packaging_enabled),
                    stock_now_packs_est: Number((store as any).stock_now_packs_est || 0),
                    min_stock_packs_est: Number((store as any).min_stock_packs_est || 0),
                    need_net_packs_est: Number((store as any).need_net_packs_est || 0)
                };
            });
    ...
    return (
        <div className="flex flex-col h-full w-full font-sans text-text-primary bg-bg-primary min-h-screen">
            ...
            {/* рџ”Ґ CARD GRID LAYOUT */}
            <main className="flex-1 overflow-y-auto custom-scrollbar p-6">
                {viewMode === 'products' ? (
                    /* PRODUCTS VIEW */
                    <div className="w-full max-w-[1700px] mx-auto grid grid-cols-[repeat(auto-fit,minmax(260px,1fr))] gap-3 pb-20 mt-4 justify-items-stretch">
                        {products.map(product => {
                            ...
                            return (
                                <div
                                    ...
                                    className="bg-white p-4 rounded-xl transition-all flex flex-col gap-2 group hover:shadow-md border border-slate-200 hover:border-blue-200 cursor-pointer min-h-[170px] relative overflow-hidden"
                                >
                                    ...
                                    <div className="flex flex-col items-center relative z-10 pt-1 min-h-[42px]">
                                        <div className={`absolute top-0 right-0 w-2 h-2 rounded-full ${dotColorClass} flex-shrink-0`}></div>
                                        <h3 className="text-sm font-bold text-slate-900 tracking-tight group-hover:text-blue-600 transition-colors leading-[1.15] line-clamp-2 font-[family-name:var(--font-chakra)] uppercase text-center w-full px-2">
                                            {product.name}
                                        </h3>
                                    </div>
                                    ...
                                    {isOutOfStock ? (
                                        ...
                                        <>
                                            <div className="grid grid-cols-2 gap-2 my-1 relative z-10 min-h-[50px] items-end">
                                                <div className="flex flex-col">
                                                    ...
                                                    <span className={`text-2xl font-bold font-[family-name:var(--font-jetbrains)] tabular-nums ${actualColorClass} leading-none`}>
                                                        {formatStock(product.computed.totalStock, product.unit)}
                                                        <span className="text-[12px] opacity-70 ml-1 font-normal text-slate-400">{product.unit || 'С€С‚'}</span>
                                                    </span>
                                                </div>
                                                <div className="flex flex-col items-end">
                                                    ...
                                                    <span className="text-2xl font-bold text-blue-600 font-[family-name:var(--font-jetbrains)] tabular-nums leading-none">
                                                        {formatMetric(product.computed.totalRecommended, product.unit)}
                                                        <span className="text-[12px] opacity-70 ml-1 font-normal text-slate-400">{product.unit || 'С€С‚'}</span>
                                                    </span>
                                                </div>
                                            </div>
                                            ...
                                            <div className="space-y-2 pt-2 border-t border-slate-100 mt-auto relative z-10">
                                                <div className="flex justify-between items-center text-xs">
                                                    ...
                                                    <span className="font-medium text-slate-600 bg-slate-100 px-1.5 py-0.5 rounded font-[family-name:var(--font-jetbrains)] tabular-nums text-[10px]">
                                                        {formatMetric(product.computed.totalMinStock, product.unit)} {product.unit || 'С€С‚'}
                                                    </span>
                                                </div>
                    ...
                    /* STORES VIEW */
                    <div className="w-full max-w-[1700px] mx-auto grid grid-cols-[repeat(auto-fill,minmax(280px,1fr))] gap-3 pb-20 mt-4">
                                    ...
                                    >
                                        ...
                                        <div className="flex items-center justify-center mb-2 relative z-10">
                                            <h3 className="text-base font-semibold text-slate-900 tracking-widest leading-tight truncate px-2 uppercase font-[family-name:var(--font-chakra)] group-hover:text-blue-600 transition-colors text-center w-full">
                                                {store.storeName.replace('РњР°РіР°Р·РёРЅ ', '').replace(/"/g, '')}
                                            </h3>
                                        </div>
                                        ...
                                        <div className="grid grid-cols-3 gap-2 relative z-10">
                                            <div className="bg-slate-50 border border-slate-100 rounded-xl p-2 text-center">
                                                ...
                                                <div className={cn(
                                                    ...
                                                    {store.totalStock.toFixed(0)} <span className="text-[10px] opacity-70">РѕРґ.</span>
                                                </div>
                                            </div>
                                            <div className={cn(
                                            ...
                                            <div className="bg-blue-50/50 rounded-xl p-2 text-center border border-blue-100 flex flex-col justify-center">
                                                ...
                                                <div className="text-xl font-bold text-blue-600 leading-none font-[family-name:var(--font-jetbrains)]">
                                                    {store.totalAvgSales.toFixed(0)} <span className="text-[10px] opacity-70">РѕРґ.</span>
                                                </div>
                                            </div>
                                        </div>
