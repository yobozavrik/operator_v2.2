...
// eslint-disable-next-line @typescript-eslint/no-unused-vars
export const FloridaProductionOpsTable = ({ data, onRefresh }: Props) => {
    // 2. State
    const [days, setDays] = useState(1);
    const [isShiftMode, setIsShiftMode] = useState(false);
    const [selectedFlorida, setSelectedFlorida] = useState<string | null>(null);
    ...
    const { data: shopStats } = useSWR<any[]>(
        selectedFlorida ? `/api/florida/shop-stats?florida=${encodeURIComponent(selectedFlorida)}` : null,
    ...
    const handleGenerateOrder = async () => {
        ...
        try {
            ...
            const response = await fetch(`/api/florida/order-plan?${queryParams}`);
    ...
    return (
        <div className="h-full flex flex-col bg-[#0B0E14] overflow-hidden font-sans">
            ...
            <div className="px-8 py-6 bg-[#141829] border-b border-white/5 shrink-0 relative flex items-center gap-10 z-20 shadow-[0_4px_20px_rgba(0,0,0,0.5)] transition-all">
                ...
                {/* Notification Toast */}
                {notification && (
                    <div className="absolute top-2 right-8 bg-[#F43F5E]/20 text-[#F43F5E] border border-[#F43F5E]/30 px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest animate-in fade-in slide-in-from-top-2 z-30 shadow-[0_0_15px_rgba(0,212,255,0.3)] backdrop-blur-md">
                        {notification}
                    </div>
                )}
                ...
                <div className="flex items-center gap-4 group/input z-10">
                    <div className="flex flex-col">
                        <label className={cn(
                            ...
                            isShiftMode ? "text-white/20" : "text-white/40 group-hover/input:text-[#F43F5E]"
                        )}>
                            Планування
                        </label>
                            ...
                            <input
                                ...
                                className={cn(
                                    ...
                                    isShiftMode
                                        ...
                                        : "focus:border-[#F43F5E] focus:shadow-[0_0_15px_rgba(0,212,255,0.3)] hover:border-white/20 shadow-inner"
                                )}
                            />
                            <span className={cn(
                ...
                <div className="flex flex-col z-10">
                    ...
                    >
                        <div className={cn(
                            ...
                            isShiftMode ? "bg-[#F43F5E] shadow-[0_0_10px_rgba(0,212,255,0.5)]" : "bg-white/10"
                        )}>
                            <div className={cn(
                ...
                <button
                    ...
                    disabled={isLoading}
                    className={cn(
                        "relative z-10 group overflow-hidden flex items-center gap-4 px-10 py-4 bg-[#F43F5E] hover:bg-[#33DDFF] active:scale-95 text-[#0B0E14] font-black uppercase tracking-[0.2em] text-sm rounded-2xl transition-all shadow-[0_0_20px_rgba(0,212,255,0.3)] disabled:opacity-50 disabled:cursor-not-allowed",
                    ...
                    <div className="absolute inset-0 flex items-center justify-center bg-[#0B0E14]/80 z-30 backdrop-blur-sm">
                        <div className="flex flex-col items-center gap-6">
                            <div className="relative">
                                <Calculator className="animate-[spin_3s_linear_infinite] text-[#F43F5E] relative z-10 drop-shadow-[0_0_15px_rgba(0,212,255,0.8)]" size={64} />
                                <div className="absolute inset-0 bg-[#F43F5E]/20 blur-xl rounded-full animate-pulse" />
                            </div>
                            <div className="flex flex-col items-center">
                                ...
                                <span className="text-[10px] font-bold uppercase tracking-widest text-[#F43F5E]/60 mt-2">Формування оптимального замовлення</span>
                            </div>
                        </div>
                    </div>
                )}
                            ...
                            .map(([day, items]: [string, any]) => (
                                <div key={day} className="flex flex-col gap-4">
                                    ...
                                    <div className="flex items-center gap-6 px-2 mb-2">
                                        <div className="px-6 py-2 rounded-xl bg-[#F43F5E]/10 border border-[#F43F5E]/30 text-white text-sm font-black uppercase tracking-[0.3em] shadow-[0_0_15px_rgba(0,212,255,0.2)] backdrop-blur-sm">
                                            День <span className="text-[#F43F5E] ml-1 drop-shadow-[0_0_5px_rgba(0,212,255,0.8)]">{day}</span>
                                        </div>
                                        <div className="h-px bg-gradient-to-r from-[#F43F5E]/50 to-transparent flex-1" />
                                    ...
                                    <div className="flex gap-4 overflow-x-auto pb-6 px-1 custom-scrollbar snap-x">
                                        ...
                                        {items.sort((a: any, b: any) => (Number(b.p_avg) || 0) - (Number(a.p_avg) || 0)).map((item: any, idx: number) => {
                                            const isSelected = selectedFlorida === item.p_name;
                                            ...
                                            return (
                                                <div
                                                    key={idx}
                                                    onClick={() => setSelectedFlorida(prev => prev === item.p_name ? null : item.p_name)}
                                                    className={cn(
                                                        ...
                                                        isSelected
                                                            ? "ring-2 ring-[#F43F5E] border-[#F43F5E] shadow-[0_0_30px_rgba(0,212,255,0.3)] scale-[1.02]"
                                                            : "border-white/5 hover:border-white/20 hover:shadow-[0_4px_20px_rgba(0,0,0,0.5)]"
                                                    )}
                                                >
                                                    {isSelected && <div className="absolute inset-0 bg-[#F43F5E]/5 pointer-events-none" />}
                                                    ...
                                                    <div className="p-4 relative z-10">
                                                        <div className="flex items-start justify-between gap-3 mb-4">
                                                            <h4 className="text-[11px] font-black text-white/90 leading-tight tracking-tight uppercase group-hover/card:text-[#F43F5E] transition-colors truncate" title={item.p_name}>
                                                                {item.p_name}
                                                            </h4>
                                                            ...
                                                            <div className="flex flex-col">
                                                                ...
                                                                <div className="flex items-baseline gap-1">
                                                                    <span className={cn(
                                                                        ...
                                                                        pOrder > 0 ? "text-[#F43F5E] drop-shadow-[0_0_15px_rgba(0,212,255,0.5)]" : "text-white/20"
                                                                    )}>
                                                                        {pOrder.toFixed(0)}
                                                                    </span>
                                                                    <span className="text-[10px] font-black text-[#F43F5E]/50 uppercase">{item.unit || 'шт'}</span>
                                                                </div>
                                                            </div>
                                                        ...
                                                        <div className="flex items-center justify-between text-[9px] font-bold text-white/40 uppercase tracking-widest pt-1 border-t border-white/5">
                                                            ...
                                                            <ChevronDown size={12} className={cn("transition-transform duration-500", isSelected && "rotate-180 text-[#F43F5E]")} />
                                                        </div>
                                                    </div>
                                                </div>
                                            );
                                    ...
                                    {/* FULL GRID ANALYTICS (Appears when a florida is selected) */}
                                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                                    {selectedFlorida && items.some((i: any) => i.p_name === selectedFlorida) && (
                                        <div className="mt-2 bg-[#141829]/50 backdrop-blur-xl border border-[#F43F5E]/20 rounded-3xl p-8 animate-in fade-in slide-in-from-top-4 duration-500 shadow-[0_0_50px_rgba(0,212,255,0.05)] relative overflow-hidden">
                                            ...
                                            <div className="flex items-baseline justify-between mb-8 pb-4 border-b border-white/5 relative z-10">
                                                <div className="flex items-center gap-4">
                                                    <div className="w-2 h-8 bg-[#F43F5E] rounded-full shadow-[0_0_15px_rgba(0,212,255,0.8)]" />
                                                    <h3 className="text-xl font-black uppercase tracking-[0.2em] text-white drop-shadow-[0_0_10px_rgba(255,255,255,0.3)]">
                                                        Аналітика: <span className="text-[#F43F5E]">{selectedFlorida}</span>
                                                    </h3>
                                                </div>
                                            ...
                                            {!shopStats ? (
                                                <div className="py-20 flex flex-col items-center gap-6 relative z-10">
                                                    <div className="relative">
                                                        <Activity className="animate-spin text-[#F43F5E] relative z-10 drop-shadow-[0_0_15px_rgba(0,212,255,0.8)]" size={48} />
                                                        <div className="absolute inset-0 bg-[#F43F5E]/20 blur-xl rounded-full animate-pulse" />
                                                    </div>
                                                    <span className="text-xs font-black uppercase tracking-[0.3em] text-[#F43F5E]/60">Отримання даних по магазинах...</span>
                                                </div>
                                            ) : (
                                                            ...
                                                            return (
                                                                <div key={sIdx} className={cn(
                                                                    ...
                                                                    <div className="flex flex-col mb-4 relative z-10">
                                                                        <h5 className="text-xs font-black uppercase tracking-tight text-white/90 group-hover/shop:text-[#F43F5E] transition-colors mb-2 truncate" title={stat.spot_name}>
                                                                            {stat.spot_name}
                                                                        </h5>
                                                                        <div className="flex items-center gap-2">
                                                                            <span className="text-[9px] font-black text-white/40 uppercase tracking-widest">СЕР.ПРОДАЖІ:</span>
                                                                            <span className="text-sm font-mono font-black text-[#F43F5E]">
                                                                    ...
                                                                    <div className="flex items-end justify-between pt-4 border-t border-white/5 relative z-10">
                                                                        <div className="flex flex-col">
                                                                            ...
                                                                            <span className={cn("text-xl font-mono font-black leading-none", isDeficit ? "text-[#FF6B6B] drop-shadow-[0_0_8px_rgba(255,107,107,0.5)]" : "text-emerald-500 drop-shadow-[0_0_8px_rgba(16,185,129,0.3)]")}>
                                                                                {stat.stock_now || 0}
                                                                                <span className="text-[10px] opacity-70 ml-1 font-sans">{getFloridaUnit(selectedFlorida)}</span>
                                                                            </span>
                                                                        </div>
                                                                        <div className="flex flex-col items-end">
                                                                            <span className="text-[8px] font-black text-[#F43F5E]/50 uppercase mb-1">ТРЕБА</span>
                                                                            <span className="text-2xl font-mono font-black text-[#F43F5E] leading-none drop-shadow-[0_0_10px_rgba(0,212,255,0.4)]">
                                                                                {need.toFixed(0)}
                                                                                <span className="text-[10px] opacity-70 ml-1 font-sans">{getFloridaUnit(selectedFlorida)}</span>
                                                                            </span>
                                                                        </div>
