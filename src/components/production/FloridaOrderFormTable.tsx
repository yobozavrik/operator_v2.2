...
interface Props {
    data: ProductionTask[]; // From v_florida_distribution_stats
    onRefresh: () => void;
}
...
export const FloridaOrderFormTable = ({ data, onRefresh }: Props) => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const res = await fetch('/api/florida/create-order', {
                method: 'POST',
                        ...
                        <div className="grid grid-cols-5 gap-2 h-full">
                            {analytics.top5.map((item, idx) => (
                                <div key={idx} className="bg-[#0B0E14] rounded p-2 border border-white/5 flex items-center justify-between group hover:border-[#E74856]/30 transition-colors">
                                    <div className="text-xs font-bold text-white/80 group-hover:text-[#E74856] transition-colors truncate max-w-[100px]" title={item.florida_name}>
                                        {item.florida_name}
                                    </div>
                                    <div className="text-right leading-none">
                                        <div className={cn("font-mono font-black", item.shop_stock <= 0 ? "text-[#E74856]" : "text-white")}>
                                            {item.shop_stock} {getFloridaUnit(item.florida_name)}
                                        </div>
                        ...
                        <div className="text-right font-mono font-black text-lg text-[#FF6B6B]">
                            {row.networkDeficit > 0 ? (
                                <span className="drop-shadow-[0_0_10px_rgba(255,107,107,0.4)]">
                                    {row.networkDeficit}
                                    <span className="text-[10px] ml-1 opacity-60 font-sans tracking-normal">{getFloridaUnit(row.name)}</span>
                                </span>
