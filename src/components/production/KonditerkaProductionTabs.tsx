...
import { KonditerkaPowerMatrix } from '../KonditerkaPowerMatrix';
import { KonditerkaProductionOpsTable } from './KonditerkaProductionOrderTable';
...
import { KonditerkaDistributionControlPanel } from './KonditerkaDistributionControlPanel';
import KonditerkaProductionSimulator from './KonditerkaProductionSimulator';
import { KonditerkaHistoricalProduction } from './KonditerkaHistoricalProduction';
                ...
                {isLoading ? (
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead className="bg-bg-primary text-[10px] uppercase font-bold tracking-widest text-text-secondary border-b border-panel-border">
                            <tr>
                                <th className="p-4">Кондитерка</th>
                                ...
                                <tr key={i} className="group hover:bg-bg-primary transition-colors">
                                    ...
                                    <td className="p-4 text-right">
                                        <span className="inline-flex items-center justify-center px-4 py-1.5 rounded-lg bg-accent-primary/10 text-accent-primary font-mono text-sm font-black min-w-[4rem] border border-accent-primary/20">
                                            {item.baked_at_factory} <span className="text-[10px] ml-1 opacity-70 lowercase">{item.unit || getKonditerkaUnit(item.product_name)}</span>
                                        </span>
                                    </td>
                                </tr>
                            ))}
...
export const KonditerkaProductionTabs = ({ data, onRefresh, showTabs = true }: Props) => {
    // UPDATED TABS: 'matrix' replaces old 'distribution', 'logistics' is NEW
    const [activeTab, setActiveTab] = useState<'orders' | 'matrix' | 'production' | 'catalog' | 'logistics' | 'simulator'>('matrix');
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    ...
    // Auto-sync once on first page load so dashboard is not stale until manual click.
    React.useEffect(() => {
        ...
        <div className="flex flex-col h-full w-full font-sans">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <div className="flex flex-col justify-center">
                                    <h1 className="text-2xl font-bold uppercase tracking-wide leading-none text-text-primary font-[family-name:var(--font-chakra)]">ЦЕХ КОНДИТЕРКА</h1>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3 my-1">
                        ...
                        >
                            ...
                            <div className="grid grid-cols-2 gap-2 mt-1 relative z-10">
                                <div>
                                    <div className="text-[9px] text-orange-500 font-bold mb-0.5 uppercase tracking-wider">🍰 Десерти</div>
                                    <div className="flex items-baseline gap-1">
                                        <span className="text-2xl font-bold tracking-tight text-slate-900">
                                            {globalMetrics.konditerka.produced.toLocaleString()}
                                        </span>
                                        <span className="text-[9px] text-slate-400 font-medium font-mono">од.</span>
                                    </div>
                                </div>
                                <div className="border-l border-slate-100 pl-3">
                                    <div className="text-[9px] text-blue-500 font-bold mb-0.5 uppercase tracking-wider">🍦 Морозиво</div>
                                    <div className="flex items-baseline gap-1">
                                        <span className="text-2xl font-bold tracking-tight text-slate-900">
                                            {globalMetrics.morozivo.produced.toLocaleString()}
                                        </span>
                                        <span className="text-[9px] text-slate-400 font-medium font-mono">шт.</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="rounded-2xl border border-slate-200 bg-white p-4 relative overflow-hidden group hover:border-blue-200 hover:shadow-sm transition-all duration-200">
                            ...
                            <div className="grid grid-cols-2 gap-2 mt-1">
                                <div>
                                    <div className="text-[9px] text-orange-500 font-bold mb-0.5 uppercase tracking-wider">🍰 Десерти</div>
                                    <div className="flex items-baseline gap-1">
                                        <span className={cn("text-2xl font-bold tracking-tight ", globalMetrics.konditerka.index >= 96 ? "text-emerald-500" : globalMetrics.konditerka.index >= 80 ? "text-amber-500" : "text-red-500")}>
                                ...
                                <div className="border-l border-slate-100 pl-3">
                                    <div className="text-[9px] text-blue-500 font-bold mb-0.5 uppercase tracking-wider">🍦 Морозиво</div>
                                    <div className="flex items-baseline gap-1">
                                        <span className={cn("text-2xl font-bold tracking-tight ", globalMetrics.morozivo.index >= 96 ? "text-emerald-500" : globalMetrics.morozivo.index >= 80 ? "text-amber-500" : "text-red-500")}>
                    ...
                    {showTabs && (
                        <div className="flex items-center gap-1 p-1 bg-bg-primary rounded-xl border border-panel-border">
                            ...
                            >
                                <div className={cn(
                                    "p-1.5 rounded-md transition-colors",
                                    activeTab === 'catalog' ? "bg-blue-500/10 text-blue-500" : "bg-transparent"
                                )}>
            ...
            <div className="flex-1 overflow-hidden relative">
                ...
                {(showTabs && activeTab === 'logistics') && (
                    <KonditerkaDistributionControlPanel />
                )}
                {(showTabs && activeTab === 'catalog') && (
                    <KonditerkaHistoricalProduction />
                )}
                {(showTabs && activeTab === 'simulator') && (
                    <KonditerkaProductionSimulator />
                )}
            </div>
