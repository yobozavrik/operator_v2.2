...
export const BulvarOrderFormTable = ({ data, onRefresh }: Props) => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const res = await fetch('/api/bulvar/create-order', {
                method: 'POST',
                        ...
                        <div className="grid grid-cols-5 gap-2 h-full">
                            {analytics.top5.map((item, idx) => (
                                <div key={idx} className="bg-[#0B0E14] rounded p-2 border border-white/5 flex items-center justify-between group hover:border-[#E74856]/30 transition-colors">
                                    <div className="text-xs font-bold text-white/80 group-hover:text-[#E74856] transition-colors truncate max-w-[100px]" title={item.bulvar_name}>
                                        {item.bulvar_name}
                                    </div>
