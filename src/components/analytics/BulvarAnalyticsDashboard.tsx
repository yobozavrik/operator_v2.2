...
interface BulvarAnalyticsDashboardProps {
    data: ProductionTask[];
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    trends?: any[];
}

export const BulvarAnalyticsDashboard: React.FC<BulvarAnalyticsDashboardProps> = ({ data, trends = [] }) => {
    ...
    const kpis = useMemo(() => {
        ...
        data.forEach(product => {
            // Because data is already transformed by transformBulvarData, 
    ...
    const categoryData = useMemo(() => {
        ...
        data.forEach(p => {
            ...
            if (isWeight) weightCount += demandValue;
            else pieceCount += demandValue;
        });

        return [
            { name: 'Штучні вироби (шт)', value: Number(pieceCount.toFixed(1)), fill: COLORS[0] },
