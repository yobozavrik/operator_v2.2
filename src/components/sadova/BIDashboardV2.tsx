...
export function BIDashboardV2() {
    ...
    const { data: deficitData, mutate: mutateDeficit } = useSWR<SupabaseDeficitRow[]>('/api/sadova/deficit', fetcher, { refreshInterval: 60000 });
    const { data: metrics, mutate: mutateMetrics } = useSWR<BI_Metrics>('/api/sadova/metrics', fetcher, { refreshInterval: 60000 });
    ...
    useEffect(() => {
        ...
        async function loadCachedProduction() {
            try {
                ...
                const res = await fetch('/api/sadova/production-daily', { headers, credentials: 'include' });
                if (!res.ok) {
            ...
            <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                <div className="flex flex-col gap-6 xl:flex-row xl:items-start xl:justify-between">
                    <div className="max-w-2xl">
                        ...
                        <h2 className="mt-3 text-3xl font-bold tracking-tight text-slate-900">Операційний огляд Sadova</h2>
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                        ...
                        <button
                            onClick={() => router.push('/sadova/distribution')}
                ...
                <div className="mt-4 flex flex-wrap gap-3">
                    <button
                        onClick={() => router.push('/sadova/stores')}
                        className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-700 transition-colors hover:bg-slate-50"
                    >
                        <MapPinned size={16} />
                        До магазинів
                    ...
                    <button
                        onClick={() => router.push('/sadova/analytics')}
                        className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-700 transition-colors hover:bg-slate-50"
                    >
                        <Factory size={16} />
                        До аналітики
