...
import type { ProductionSnapshotItem } from '@/components/sadova/SadovaDistributionPanel';
...
export function SadovaDeliveryConfirm({
    onRunDistribution,
    onExportExcel,
    runDisabled = false,
    ...
    const loadData = async (date = selectedDate) => {
        ...
        try {
            const [shopsRes, confirmRes] = await Promise.all([
                fetch('/api/sadova/shops'),
                fetch(`/api/sadova/confirm-delivery?date=${date}`),
            ]);
    ...
    const handleConfirm = async () => {
        ...
        setMessage(null);
        try {
            const response = await fetch('/api/sadova/confirm-delivery', {
                method: 'POST',
