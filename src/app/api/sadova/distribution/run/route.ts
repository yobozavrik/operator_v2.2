...
export async function POST(request: Request) {
    try {
        ...
        try {
            const edgeManufactures = extractSadovaEdgeProduction(liveEdgePayload, 2);
            if (edgeManufactures.length === 0) {
                ...
                catalogSync = await syncSadovaCatalogFromManufactures(sadovaDb, categoriesDb, rawManufactures, 2);
            }
        } catch (err: any) {
        ...
        (catalogRaw || []).forEach((c: any) => {
            const norm = normalizeSadovaName(c.product_name);
        ...
        if (stocksToInsert.length > 0) {
            const { error: stocksInsertError } = await sadovaDb
        ...
        if (manufacturesToInsert.length > 0) {
            const { error: prodInsertError } = await sadovaDb
        ...
        // 9. Insert Meta
        const { error: metaInsertError } = await sadovaDb
        ...
        const { data: debtSnapshot } = await sadovaDb
            .from('delivery_debt')
        ...
        const { error: runError } = await sadovaDb.rpc('fn_orchestrate_distribution_live', {
            p_batch_id: batchId,
