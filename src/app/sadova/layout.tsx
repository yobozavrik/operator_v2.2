...
const NAV_ITEMS = [
    { href: '/sadova', label: 'Огляд', icon: BarChart2 },
    { href: '/sadova/distribution', label: 'Розподіл', icon: Truck },
    { href: '/sadova/debt', label: 'Борг', icon: AlertTriangle },
    { href: '/sadova/stores', label: 'Магазини', icon: MapPinned },
    { href: '/sadova/analytics', label: 'Аналітика', icon: Factory },
];

export default function SadovaLayout({ children }: { children: React.ReactNode }) {
                        ...
                        <div className="flex flex-col gap-3 xl:items-end">
                            ...
                            <nav className="flex flex-wrap items-center gap-2">
                                {NAV_ITEMS.map(({ href, label, icon: Icon }) => {
                                    const active = pathname === href || (href !== '/sadova' && pathname.startsWith(href));
                                    return (
                                        <Link
                                            key={href}
                                            href={href}
