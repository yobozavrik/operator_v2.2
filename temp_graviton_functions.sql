...
CREATE OR REPLACE FUNCTION graviton.fn_worker_distribute_product(
    ...
    p_batch_id uuid,
    p_shop_ids integer[] DEFAULT NULL, p_target_date date DEFAULT p_target_date
)
RETURNS void
LANGUAGE plpgsql
AS $$
...
BEGIN
    ...
    WHERE business_date = p_target_date;
    ...
    LOOP
        -- Call Worker
        PERFORM graviton.fn_worker_distribute_product(r.product_id, v_batch_id, p_shop_ids, p_target_date);
        v_count := v_count + 1;
    END LOOP;
